...
@app.before_request
def prepare_sentiment_quote_nlp():
    global quotes

    # load the quote stash
    df = pd.read_csv(r'C:\Users\monic\Downloads\quotes_nlp.csv')

    sid = SentimentIntensityAnalyzer()

    all_compounds = []
    for sentence in df['quote']:
    ...
    df['sentiment_score'] = all_compounds

    # create ladder index
    df = df.sort_values('sentiment_score')
    df['index'] = [ix for ix in range(0, len(df['quote']))]
...
@app.route("/")
def quote_me():
    quote_nlp_tmp = df.copy()
    max_index_value = np.max(quote_nlp_tmp['index'].values)
    ...
    # grab a random value
         ...
         random_quote = quote_nlp_tmp.iloc[new_index]
